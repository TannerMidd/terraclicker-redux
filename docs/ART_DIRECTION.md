...
Phase 2 adds three more, same standard:

- **The sky is arithmetic.** Weather is a pure function of (seed, type, dir, aspects, gameTimeMs) — `engine/weather.ts` — quantised into drifting fronts, gated by the gauges (no Atmo, no rain; vacuum gets meteor showers), and shared by both scenes: the cell on the orbital cloud shell is the front the walker stands in. Weather bends decisions — scanner reach, the marker rail, buried seams, swing counts — and never touches anything resembling a health bar.
- **Water participates.** Graded depth instead of a wall: wade to the thigh at falling speed, then a buoyant refusal up the seabed gradient until the skimmer exists. Lava refuses at the ankle. Shore breaks and waterline mist live in the liquid shader, driven by depth like everything else.
- **Landmarks are the lattice, coarser.** Two to four authored kinds per planet type on ~1.9 km cells (`surface/surfaceLandmarks.ts`), compass-named in sight range, one region ≈ seven instanced draws. The award-winning fjords appear where the quirk holds — and the quirk now crinkles the surface macro exactly as it crinkles orbit, which is the continent law holding on worlds where it quietly wasn't.
