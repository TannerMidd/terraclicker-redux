...
/** Where bought refit hardware hangs on the chase-view hull (local frame). */
const EXTERIOR_PODS: RefitPodSpec[] = [
  { id: 'skimmer', position: [0, 0.2, 0.34], height: 0.09 },
  { id: 'cargoHold', position: [0, -0.1, 0.18], height: 0.16 },
  { id: 'rigBay', position: [0, -0.095, -0.2], height: 0.13 },
  { id: 'deterrent', position: [0, 0.27, 0.05], height: 0.07 },
  { id: 'atmo', position: [0, 0.02, -0.38], height: 0.09 },
];
...
export function RunaboutExterior() {
  ...
  // The authored hull (3.1), fitted to the hand-built silhouette's exact
  // envelope so the chase camera, the collision space and the flight scale
  // checks all keep the ship they were tuned against. This component mounts
  // at app start — usually BEFORE the GLB lands — so it upgrades once when
  // the kit arrives instead of staying boxes for the whole session.
  const kitHull = useKitGeometry(RUNABOUT_KIT, () =>
    runaboutGeometry({ min: [-0.76, -0.12, -0.85], max: [0.76, 0.26, 0.79] }),
  );
  ...
  return (
    <group ref={root} name="runabout-exterior" visible={false} scale={MODEL_SCALE}>
      {kitHull ? (
        <>
          {/* One authored hull, panel lines and gear included (3.1); the
              bought refit hardware hangs off it (3.2). */}
          <mesh geometry={kitHull} material={shipMaterial()} />
          <RefitPods pods={EXTERIOR_PODS} />
        </>
      ) : (
        <>
