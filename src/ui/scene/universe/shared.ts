...
export function focusOn(target: FocusTarget): void {
  ...
  const galaxyish = target.kind === 'galaxy' || isConsumed(target.index);
  bus.setZoom(galaxyish ? BAND_STOPS[3] : BAND_STOPS[2]);
  bus.setFocus(target);
  audio.zoomWhoosh(1, galaxyish ? 3 : 2);
}

/** Step one level out: system-in-a-galaxy → its galaxy → the open journey. */
export function stepFocusOut(): void {
  const bus = useUiBus.getState();
  const f = bus.focus;
  if (!f) return;
  if (f.kind === 'system' && isConsumed(f.index)) {
