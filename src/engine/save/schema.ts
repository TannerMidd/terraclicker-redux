  ...
  expedition: z.object({
    ...
    groundWorlds: z.record(
      ...
      z.object({
        ...
        landmarks: z.record(z.string(), z.number().int().min(0)),
        weather: z.record(z.string(), z.number().int().min(0)),
        civicVisits: z.number().int().min(0),
        familiarity: z.number().int().min(0),
        atlasCompletedAtMs: z.number().min(0).nullable(),
        projectSites: z.record(
          z.string(),
          z.object({
            id: z.string(),
            kind: z.enum([
            ...
            state: z.enum(['scaffold', 'complete']),
            atMs: z.number().min(0),
            sourceWorld: z.number().int().min(1),
          }),
        ),
        marks: z.array(
          z.object({
    ...
    fieldProjects: z.record(
      ...
      z.object({
        ...
        systemIndex: z.number().int().min(0),
        receiver: z.number().int().min(1),
        source: z.number().int().min(1),
        stage: z.enum(['investigate', 'source', 'return', 'complete']),
        startedAtMs: z.number().min(0),
        updatedAtMs: z.number().min(0),
    ...
    routes: z.record(
      ...
      z.object({
        ...
        name: z.string(),
        establishedAtMs: z.number().min(0),
        trips: z.number().int().min(0),
      }),
    ),
    groundCheckpoints: z.record(
      z.string(),
      z.object({
        savedAtMs: z.number().min(0),
        haul: z.array(
          z.object({
            kind: z.string(),
            n: z.number().int().min(1),
            method: z.enum(['quick', 'core', 'prospect']),
          }),
        ),
        sites: z.record(
          z.string(),
          z.enum(['worked', 'prospected', 'preserved', 'visited']),
        ),
        ...
        evidence: z.object({
          ...
          readings: z.number().int().min(0).optional(),
          readingDirs: z.array(
            z.tuple([z.number(), z.number(), z.number()]),
          ).max(3).optional(),
          chartedIds: z.array(z.string()).max(256).optional(),
          weathered: z.array(z.string()).optional(),
          marks: z.array(
            z.object({
              kind: z.enum(['beacon', 'station', 'repair', 'shelter']),
              dir: z.tuple([z.number(), z.number(), z.number()]),
          ...
          buriedWorked: z.boolean().optional(),
          lead: z.boolean().optional(),
          flew: z.boolean().optional(),
          setdowns: z.number().int().min(0).optional(),
          charted: z.number().int().min(0).optional(),
          rangeM: z.number().min(0).optional(),
        }),
      }),
    ),
